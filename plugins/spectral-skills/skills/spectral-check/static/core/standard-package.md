...
Spectral-check input is the standard package written by `spectral-reader`.
...
When spectral-check writes a modified dataset, it writes the same filenames to a new output
directory under `cleaned_package/`. Do not create a second handoff format.

Observation-only spectral-check writes only `qc_result.json` by default.
...
Before any spectral-check computation, verify:
...
If package integrity fails, block spectral-check execution and send the user back to reader or ask for
a corrected package. Do not repair reader-level structural errors silently.

## Contract Updates

Keep spectral-check additions compact and downstream-useful:

- record that the package has passed spectral-check checks in `qc_result.json` or was
