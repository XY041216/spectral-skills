...
description: >-
  ...
  spectral quality checks such as package consistency, missing or illegal values, spectral
  ...
  train models, or develop the spectral-check skill itself.
---

# Spectral Check

`spectral-check` performs pre-modeling quality checks on the standard spectral
...
Use this skill only for quality checks on an existing standard spectral
package:
...
`reader package -> run standard quality check -> write qc_result.json -> summarize risks -> ask confirmation only for destructive cleaning -> optionally write cleaned_package/`

Default behavior is conservative: check and mark only. `spectral-check` does not
...
perform limited quality-check cleaning and write a new standard package for downstream
skills.

Use one executable entry for deterministic work:

```bash
python skills/spectral-check/scripts/qc_spectral_package.py --package-dir <reader-output> --mode check --output-dir <qc-output> --json
```

Modes:

- `check`: run the standard non-destructive quality-check scheme.
- `mark`: run the standard quality-check scheme and optional advanced method-pool outlier
...
Do not output a PCA feature package from spectral-check.
...
quality-check strategy; HR/MCCV were not run by default. I can rerun mark mode with MD,
...
- G. Intersection of spectral-check outliers and HR/MCCV risk.
...
Spectral-check may mark suspected mislabels, but it must not automatically modify labels.
...
Confirmed cleaning is limited to bounded quality-check actions:
...
After spectral-check, summarize the result in a few sentences:

`Quality check complete. Data have 120 samples and 3401 bands. No missing values, illegal values, constant bands, or low-variance bands were found. No high-confidence outliers were found; 2 medium-risk samples were marked. The data can enter stratified splitting.`

For warning-only spectral-check, explicitly say it does not block the next step:

`Quality-check status is warning, not blocked. The package can enter splitter. Review high-confidence outliers if you want cleaner data, but do not delete medium-confidence or spike candidates without confirmation.`

For blocking issues, lead with the blocker:

`Quality-check blocked downstream use: X.csv has 3401 columns but band_axis.csv has 3400 rows. Fix the standard package before splitting or modeling.`
...
`Quality-check warning: many spectra have highly similar global shape, including cross-label pairs. This more likely indicates class overlap or subtle local spectral differences than duplicated samples. No automatic deletion is recommended.`

Do not dump file internals unless the user asks.

## Read As Needed

- Use `static/core/check-boundary.md` for scope and neighboring-skill boundaries.
...
- Use `static/fragments/method-pool.md` for standard quality-check method families.
- Use `static/fragments/missing-values.md` for missing-value quality-check behavior.
...
- Use `references/check-scenarios.md` for common user intents and expected behavior.
