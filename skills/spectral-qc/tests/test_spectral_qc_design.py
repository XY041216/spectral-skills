...
QC_DIR = REPO_ROOT / "skills" / "spectral-qc"
...
def test_spectral_qc_skill_files_exist() -> None:
    required = [
        "SKILL.md",
        "manifest.yaml",
        ...
        "references/qc-scenarios.md",
        "references/method-selection-cases.md",
        "references/handoff-examples.md",
        "schemas/qc_action_result.schema.json",
    ]
    for rel in required:
        assert (QC_DIR / rel).exists(), rel


def test_spectral_qc_frontmatter_and_manifest_boundary() -> None:
    skill = _read(QC_DIR / "SKILL.md")
    assert "name: spectral-qc" in skill
    ...
    manifest = yaml.safe_load(_read(QC_DIR / "manifest.yaml"))
    assert manifest["name"] == "spectral-qc"
    assert manifest["skill_type"] == "quality control"
    assert "reading raw spectral source files" in manifest["activation_boundary"]["not_for"]
    assert "spectral quality control" in manifest["activation_boundary"]["use_for"]
...
def test_confirmation_gate_protects_destructive_actions() -> None:
    text = _read(QC_DIR / "static/core/confirmation-gates.md")
    for phrase in [
...
def test_method_pool_contains_required_families() -> None:
    text = _read(QC_DIR / "static/fragments/method-pool.md")
    for phrase in [
        "NOE",
        "MD",
        "PCA Hotelling T2",
...
def test_qc_knowledge_does_not_create_report_or_debug_system() -> None:
    ...
    for path in [*QC_DIR.glob("**/*.md"), *QC_DIR.glob("**/*.yaml")]:
        text = _read(path)
...
def test_qc_action_result_schema_is_valid_json() -> None:
    payload = json.loads(_read(QC_DIR / "schemas/qc_action_result.schema.json"))
    assert payload["title"] == "Spectral QC Action Result"
